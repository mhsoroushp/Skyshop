...
export class ShowBasketService {
    ...
    basketItemCount = computed(() => {
        return this.basketItems()?.reduce((count, item) => count + item.quantity, 0);
    });

    getBasket(){
        this.http.get<BasketItem[]>(this.baseUrl, {
            withCredentials: true
        }).subscribe({
            next: (basket) => {
                console.log('Fetched basket items:', basket);
                this.basketItems.set(basket);
            },
            error: (err) => {
    ...
    updateQuantity(productId: number, quantity: number): Observable<any> {
        ...
        return this.http.put<any>(`${this.baseUrl}/product/${productId}/quantity`, request, {
            withCredentials: true
        }).pipe(
            map((response) => {
